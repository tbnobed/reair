# Re-Air Review System design system
...
workspace package (`@workspace/reair-review-system`): other artifacts depend
...
This system was extracted from the Re-Air Guided Review mockup in the workspace.
The source is an application-UI reference rather than a pre-existing component
library, so the standard component catalog is themed from its visual language.
No standalone logo asset exists in the source; retain the Re-Air name as text
rather than inventing a mark.

- Use warm archival surfaces for reading, evidence, and supporting context.
- Use broadcast-console navy for persistent navigation, queues, and decision areas.
- Reserve signal orange for active review, progress, urgency, and primary decisions.
- Use aqua for navigation or contextual references and green for cleared/safe states.
- Use IBM Plex Sans for prose, IBM Plex Sans Condensed for operational headings and
  uppercase labels, and IBM Plex Mono for IDs, timecodes, dates, counts, and metadata.
- Prefer compact, aligned information structures with thin dividers and restrained
  corner radii. Reveal secondary context progressively instead of removing it.
- Keep evidence and its decision close together. Do not use decorative color where
  it could be confused with an editorial status.
...
`@workspace/reair-review-system/components/...`. Never use a `@/` alias inside
...
for `@workspace/reair-review-system` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/reair-review-system` as a `workspace:*` dependency,
...
- Building or styling web UI: `artifacts/reair-review-system/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/reair-review-system/docs/consuming-expo.md`
- Building or styling a slide deck: `artifacts/reair-review-system/docs/consuming-slides.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/reair-review-system/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/reair-review-system/docs/migrating-expo.md`
