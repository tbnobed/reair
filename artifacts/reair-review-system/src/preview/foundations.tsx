    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="rounded-xl border bg-card p-5 text-card-foreground">
          ...
          <Card className="mt-4">
            <CardHeader>
            <CardTitle>Review archive clip</CardTitle>
              <CardDescription>
                Components composed for focused editorial decisions.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-1.5">
                <Label htmlFor="overview-name">Find a clip or person</Label>
                <Input id="overview-name" placeholder="Search the live archive" />
              </div>
              <div className="flex items-center gap-2">
                <Switch defaultChecked id="overview-notify" />
                <Label htmlFor="overview-notify">Show date-sensitive material</Label>
                <Badge className="ml-auto">Needs review</Badge>
              </div>
            </CardContent>
            <CardFooter className="gap-2">
              <Button>Record decision</Button>
              <Button variant="outline">Skip clip</Button>
            </CardFooter>
          </Card>
        </section>
      </div>
  ...
  return (
    <div className="space-y-8 rounded-xl border bg-card p-6 text-card-foreground">
      ...
      <section className="space-y-3 border-t pt-6">
        ...
        <Guidelines items={[
          { kind: 'do', text: 'Reserve signal orange for active review, urgency, progress, and primary editorial decisions.' },
          { kind: 'do', text: 'Use aqua for navigation and contextual references, and green for cleared or safe states.' },
          { kind: 'dont', text: 'Use status colors decoratively where they could be mistaken for an editorial result.' },
        ]} />
      </section>
  ...
  return (
    <div className="space-y-8 rounded-xl border bg-card p-6 text-card-foreground">
      <section>
        ...
        <p className="mt-4 font-serif text-4xl font-bold">Resolve each decision before moving on</p>
      ...
      <section className="space-y-3 border-t pt-6">
        <h2 className="font-semibold">Usage</h2>
        <Guidelines items={[
          { kind: 'do', text: 'Use condensed type for operational headings, section labels, and compact controls.' },
          { kind: 'do', text: 'Use monospace type for clip IDs, timecodes, dates, counts, and technical metadata.' },
          { kind: 'dont', text: 'Set long synopsis or evidence prose in condensed or monospace type.' },
        ]} />
      </section>
...
export function LayoutPage() {
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <section className="space-y-3 rounded-xl border bg-card p-6 text-card-foreground lg:col-span-2">
        <h2 className="font-semibold">Composition</h2>
        <Guidelines items={[
          { kind: 'do', text: 'Keep evidence and its decision controls close together in the reading flow.' },
          { kind: 'do', text: 'Use aligned, compact information structures with thin dividers and progressive disclosure for secondary context.' },
          { kind: 'dont', text: 'Create oversized empty regions that separate reviewers from the metadata they need.' },
        ]} />
      </section>
    </div>
  );
